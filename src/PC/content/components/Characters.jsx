  ...
  for (const party of parties) {
    for (const group of party.groups || []) {
      const inGroup = (group.members || []).some(
        (m) => m.id === charId || m.characterId === charCharacterId || m.id === charCharacterId || m.characterId === charId
      );
      if (inGroup) {
 ...
 */
function Characters({ characters = [], groups = [], parties = [], loading = false, addedCharacterIds, onMemoUpdate, canEditMemo = false }) {
  ...
  if (loading) {
    return (
      <div className="border-2 border-gray-200 dark:border-gray-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-4 cursor-pointer select-none" onClick={() => setSectionExpanded((e) => !e)} role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && setSectionExpanded((v) => !v)} aria-expanded={sectionExpanded}>
          <h2 className="text-2xl font-bold">Characters</h2>
          <span className="text-sm text-gray-500 dark:text-gray-400">{sectionExpanded ? "▼" : "▲"}</span>
        </div>
        {sectionExpanded && (
              ...
              <div className="flex-1 min-w-0">
                ...
                {(character.groupNum !== null && character.groupNum !== undefined) && (
                  <div className="flex flex-wrap items-center justify-center gap-1.5 mb-1">
                    <span className="text-xs px-2 py-0.5 rounded-md font-medium bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 truncate max-w-[8rem]" title={groups.find((g) => g.id === character.groupNum)?.name ?? `그룹 ${character.groupNum}`}>
                  ...
                  return (
                    <div className="flex flex-wrap items-center justify-center gap-1.5 mb-1">
                      {partyGroups.map((pg, i) => (
                        <span key={i} className="text-xs px-2 py-0.5 rounded-md font-medium bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 truncate max-w-[10rem]" title={`${pg.partyName} · ${pg.groupName}`}>
